...
This section establishes the foundational knowledge required to understand the paper's focus. It provides an overview of quantum software engineering, UML, quantum computing, and the Variational Quantum Eigensolver (VQE) algorithm, offering the reader a base understanding of these topics before exploring specific UML diagram structures in detail.
...
A single quantum aggregation relationship exists between \textbf{EstimatorV2} and \textbf{CostFunction}, represented by a double-lined edge. In this relationship, the \textbf{EstimatorV2} quantum class is passed as a parameter to \textbf{CostFunction} quantum class.
...
This section will discuss the results of analysing the two quantum UML adaptations and whether the objectives outlined in the Background section were achieved. It begins with the author’s observations and then assesses each diagram using five established characteristics for effective software modelling. The section concludes with final reflections and recommendations for future research to build upon this paper’s findings.

\subsection{Author's Analysis}

The following section addresses the author's thoughts on several aspects: analysing each diagram's construction, visibility, conveyed information, and the challenges encountered during their creation.
...
Distinguishing between classical and quantum elements is particularly valuable for cost analysis. Given the significantly higher expense of quantum hardware and information processing, identifying which system components rely on quantum resources provides essential insights into operational costs. The diagrams in this paper focus on a small code snippet with minimal quantum hardware access, limiting their effectiveness for comprehensive cost evaluation. A broader system model integrating classical and quantum hardware would offer a more complete perspective. A promising approach could involve modelling the VQE algorithm in Qiskit on a larger scale, incorporating its interactions with the Qiskit Runtime Service. This approach would align with the class diagram in the quantum UML profile, which models quantum and classical elements within a complete application\cite{Pérez-Castillo2022}. To enable a fair comparison between diagrams of a similar type, the use of the same modelling software, such as Papyrus, should also be considered.
...
This paper sought to answer the question, which quantum UML adaptation offers the best solution? Analysis shows that they both contain weaknesses and strengths with the quantum UML profile offering more specificity with Q-UML offering more flexibility. Depending on the users purpose and who they share the models with, they may need a UML adaptation which offers more detail, to share with professionals who understand quantum better, or less detail for those who just need to differinete between quantum or classical. which is the best model? can this be answered without comparing them to other models?

Whether full-scale UML should be adopted compared to another simiplied modelling approach cannot be answered without comparing these UML adpations to other model types, provide the links to the paper here. In the context of these two adaptations however, the quantum uml profile is fully compliant, and those who use full UML notation may wish to use a UML software like Papyrus which is tailored to this modelling, as using Lucidchart is time consuming and Mermaid and PUML provided unpredictable results. 

We would probably assume QUML would be better for simplified diagram, as Quantum UMl profile is tied to UML. it would be good to establish a standard however there are issues that were in the Q-UML paper - 
...
We did achieve implementing the UML adaptions into real world scenarios, discoring what is the best method for modelling UML diagrams, it offers a good basis for people wanting to understand these methods (without reading the full papers) to implement these UML adpations in their own work.

The first objective was achieved, however through analysis it may not be the case that a real-world example was the most suitable. Modelling an algorithm, rather than code, would allow us to answer the question regarding model predictiveness. 

The second objective was achieved as the sequence and class diagrams complement each other and the sequence diagram was the best to depict exhanches of messages. 

The third objective could be improved with polling, giving experts in the fields of quantum and software engineering to provide feedback, which would help answer the questions as to what would really be utilised int he real world. Throughout the paper only having a select number of peers and reddit articles to answer this question as to what people in industry actually use to model systems. Whether UML is even used that much?

The final objective was not completed, and this could lead on to some promising future work. further exploration
into modelling fundamental quantum concepts would be beneficial for evalu-
ating the effectiveness of applying UML to quantum systems.

ulitmatley, more work into this and creating more UML quantum diagram adaptations will help to refine and come up with the best method which can be implements for Q-SE. This includes everything mentioned; models of algorithms, of other code, of fundamental quantum propreities, models of VQE on a larger scale, larger scale of Qiskit, different UML algorithms, and different models. 

\section{Future Work}

Future work could also be covering all of the different UML types, not just class and sequence, though we may be limited to the examples given in the papers, however with enough context they could be expanded on by someone else. 

Plant UML is
currently the most convenient tool for creating UML diagrams, as Lucidchart is
significantly more labour-intensive. Developing an extension for Plant UML or
other open-source tools like Mermaid to incorporate Q-UML formatting would
be highly advantageous for future diagram creation. Such an extension would
also increase the utility of this method, making it more accessible for continued
exploration and broader adoption in the QSE field. If a Q-UML extension were developed for one of the available open-source UML tools,
its associated cost would be eliminated.

MDD with the quantum UMl profile
find out how to do better class diagrams using mermaid and plant uml 
