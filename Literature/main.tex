...
\author{Supervisor: Carlos A. Pérez-Delgado\\ Programme: MSc Computer Science (Artificial Intelligence) \\ Word Count: 8,072}
...
This section will detail the two different quantum adaptations to UML, which are mentioned in the Background section. 
...
The instances \textbf{backend}, \textbf{res}, \textbf{estimator} and \textbf{cost\_func} are depicted as quantum through the use of bold text for their names, double lines for their rectangle elements and double lines for their lifelines. 

The \textbf{backend} object facilitates communication between quantum and classical machines. Although the messages it sends in the diagram remain classical, its design is fundamentally quantum, therefore following the first design principle \textbf{Quantum Classes}. The \textbf{estimator} is also the instance of a quantum class that executes the ansatz circuit through its \textit{.run()} operation. The \textbf{cost\_func} object contains \textbf{estimator} in its design and, therefore, also follows the first design principle. The \textbf{res} object is a classical object that must be passed the \textbf{cost\_func} as parameters for its execution. As it contains at least one quantum element, it is upgraded to a quantum label, following the third design principle \textbf{Quantum Supremacy}. 
